Shader "Test/SimpleSurface2" {
	...
	SubShader {
		...
		void vert(inout appdata_full v) 
		{
			...
			float4 texEffect = tex2Dlod(_EffectTex, float4(v.texcoord.xy + _Time.x*_TimeFactor, 0, 0));

			v.vertex.xyz += v.normal * texEffect.r*_Bump;
		}
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 cEffect = tex2D(_EffectTex, uv + _Time.x*_TimeFactor);
